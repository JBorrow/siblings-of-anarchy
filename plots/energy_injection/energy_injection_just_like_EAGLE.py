...
def subcycle(positions, entropies, index, target_energy_addition, smoothing_length):
    ...
    energy_diff = [0.0]
    # EAGLE works on only using the central particle. That's what we'll do here.
    energy_EAGLE_believes = [
        calculate_u(
            calculate_pressure(position, smoothing_length, positions, entropies),
            entropies[index],
        )
    ]
    ...
    # EAGLE does not update based on neighbours.
    P_EAGLE = calculate_pressure(position, smoothing_length, positions, entropies)
    ...
    while (
        ...
        new_injection = target_energy - energy_EAGLE_believes[-1]

        central_particle_energy = calculate_u(P_EAGLE, entropies[index])
        ...
        central_particle_entropy_new = calculate_A(P_EAGLE, central_particle_energy_new)

        # EAGLE only updates the self-contribution. Let's do that now here.
        kernel_self_contribution = kernel(0.0, smoothing_length * kernel_gamma)
        # This gets a bit hairy, but in LaTeX:
        # P_new = \left[ (P_{old}^{1/\gamma} - A_{old}^{1 / gamma}W_0) + A_{new}^{1 / gamma} W_0 \right]
        change_in_A = central_particle_entropy_new ** (1 / gas_gamma) - entropies[
            index
        ] ** (1 / gas_gamma)
        P_EAGLE = (
            P_EAGLE ** (1 / gas_gamma) + change_in_A * kernel_self_contribution
        ) ** gas_gamma
        # So we can cache what EAGLE believes!
        new_energy_according_to_EAGLE = calculate_u(
            P_EAGLE, central_particle_entropy_new
        )
        ...
        energy_EAGLE_believes.append(new_energy_according_to_EAGLE)
